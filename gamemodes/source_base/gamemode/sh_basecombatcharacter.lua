...
function _R.Player:SwitchWeapon( pWeapon )
	...
	elseif ( hActiveWeapon:GetClass() == pWeapon ) then
		if ( not hActiveWeapon:IsWeaponVisible() ) then
			return hActiveWeapon:Deploy() -- Fix; do we need to initialize? Do we need to call both? This is a really hacky func
		end
	...
	return pWeapon:Deploy() -- Fix; need to initialize
end
