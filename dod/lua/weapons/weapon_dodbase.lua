...
-- FIXME: World model disappears when switching to a weapon with no ammo
...
if CLIENT then
	function SWEP:PlayWorldReloadSound( pPlayer )
		...
		local shootsound = self:GetSound( "reload" )
...
function SWEP:Smack()
	...
	if ( tr.Entity:IsPlayer() ) then
		if ( bit.band( self.m_iSmackDamageType, MELEE_DMG_STRONGATTACK ) ) then
			self:WeaponSound( SPECIAL1 ) -- Fix all of this
		else
			self:WeaponSound( "melee" )
		end
	else
